...
final class WorkSchedule
{
    ...
    /**
     * Расписание построчно — семь дней в порядке недели (веха 4.5).
     *
     * Это ВТОРОЕ представление того же расписания рядом с `label()`,
     * и оно здесь, а не в сервисе страницы, ровно из-за форматирования
     * времени: «09:00» человеку читается как выгрузка из системы, ноль
     * снимает приватный `humanTime()`, и собранная в чужом классе строка
     * разошлась бы с подвалом на один символ. Разошлась бы молча — оба
     * варианта выглядят правдоподобно, пока не увидишь их рядом.
     *
     * **Пустой список означает «строчить нечего», а не «данных нет».**
     * Случая два, и оба заканчиваются одной строкой вместо таблицы:
     *
     * - рабочих дней нет вовсе — блок не рендерится (правило проекта
     *   «блок без данных не рендерится»; `WARN` про это пишет `label()`,
     *   и второй здесь был бы дублем в логе на каждый рендер);
     * - вся неделя одинаковая — семь строк «9:00–21:00» подряд не несут
     *   ни одного бита сверх «Без выходных, 9:00–21:00», а именно так
     *   компания и работает сегодня. Таблица из семи повторов читается
     *   как невыполненная работа вёрстки, а не как расписание.
     *
     * В обоих случаях вызывающая сторона показывает `label()`. Разводить
     * их здесь нечем и незачем: `label()` сам отдаёт `null` в первом
     * случае и строку во втором.
     *
     * @return list<array{day: Weekday, label: string, hours: ?string}>
     */
    public function rows(): array
    {
        if (! $this->hasWorkingDays() || $this->isEveryDayAlike()) {
            return [];
        }
        ...
        foreach (Weekday::cases() as $day) {
            $hours = $this->days[$day->value] ?? null;
            ...
            $rows[] = [
                ...
                'hours' => $hours === null
                    ? null
                    : ($this->isRoundTheClock($hours) ? 'Круглосуточно' : $this->timeRange($hours)),
            ];
        }
