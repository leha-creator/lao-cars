    ...
    public static function configure(Table $table): Table
    {
        return $table
            ->columns([
                ...
                // Строка та же, что на сайте, — с «от», диапазоном
                // и уточнением: `->money()` показал бы голую нижнюю
                // границу, и таблица разошлась бы с карточкой. Без цены
                // `formatStateUsing()` не вызывается — остаётся плейсхолдер.
                // Сортировка — по нижней границе, как в каталоге.
                // `wrap()` — ради диапазона: пробелы внутри суммы
                // неразрывные, и «от 5 100 000 до 5 600 000 ₽» одной
                // строкой раздвигало колонку так, что таблица уезжала
                // в горизонтальную прокрутку. Переносится она по «до».
                TextColumn::make('price')
                    ->label('Цена')
                    ->formatStateUsing(fn (mixed $state, Car $record): string => $record->priceLabel())
                    ->wrap()
