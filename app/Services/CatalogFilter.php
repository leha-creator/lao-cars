...
final class CatalogFilter
{
    ...
    public function apply(Builder $query, CatalogCriteria $criteria): Builder
    {
        ...
        // потому что диапазоны пересекаются. Сравнение с `price` выбросило
        // бы его, хотя комплектации в нужном бюджете у него есть. Индекс
        // `cars_available_price_index` это условие не использует; каталог —
        // десятки строк, и `price_to` с сортировкой по-прежнему идут по нему.
        if ($criteria->priceFrom !== null) {
            $query->whereRaw('coalesce(cars.price_max, cars.price) >= ?', [$criteria->priceFrom]);
        }
