...
enum CarAttributeType: string implements HasLabel
{
    ...
    /**
     * Можно ли фильтровать каталог по характеристике этого типа.
     *
     * Только список и «Да / Нет». Равенство по свободному тексту в фильтре
     * бессмысленно: администратор пишет «Long Range AWD», пользователь
     * такого не наберёт. Числу нужен диапазон, то есть приведение текстовой
     * колонки к числу на каждой строке — запрос, который не покроет ни один
     * индекс.
     *
     * Диапазоны по числовым характеристикам («клиренс от 180 мм») отложены
     * с честным триггером: первый запрос заказчика на такой фильтр — сигнал
     * заводить отдельные типизированные колонки значений, а не подпирать
     * текстовую.
     */
    public function isFilterable(): bool
    {
        return $this === self::Select || $this === self::Boolean;
    }
}
