...
**Текущее состояние:** собраны каркас проекта (веха 3.1) и схема данных (веха 3.2) — модели, миграции, фабрики и сиды. Ресурсы админки и публичные страницы — в работе, см. [роадмап](.ai-factory/ROADMAP.md).
...
Динамические характеристики автомобиля («кузов», «цвет» и подобные) в схему не входят — это отдельный справочник вехи 3.3. Цена и пробег допускают `null`: это «цена по запросу» и «новый автомобиль», а не ноль.

**Демо-данные.** `php artisan db:seed` создаёт 8 марок, 12 автомобилей во всех состояниях карточки (в наличии, под заказ, продан, без цены), 21 позицию прайса по всем пяти категориям, команду, отзывы (включая один на модерации) и настройки сайта из макета. Фотографии берутся из `assets/cars/` и копируются в `storage/app/public/cars/`.
