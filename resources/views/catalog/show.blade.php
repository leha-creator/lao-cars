...
@section('content')
    ...
    <section @class([
        ...
        <div class="mx-auto grid max-w-page gap-10 lg:grid-cols-[1.4fr_1fr] lg:gap-16">
            ...
            >
                {{-- Пропорция кадра — 3:2 (правка 21.08.2026), базовая
                     на весь сайт: в этом формате снимают автомобили,
                     и у такого снимка поля `bg-photo` схлопываются
                     в ноль — кадр виден целиком, без полей и без
                     обрезки. Поля остаются у файла другого формата,
                     ради чего `object-contain` вехой 4.14 и заводился.

                     Брейкпойнта здесь больше нет, и это часть той же
                     правки: 16:10 на телефоне и 16:9 на компьютере
                     означали, что «правильный формат» у одного файла
                     разный на разной ширине экрана, то есть без полей
                     он не показывался нигде. Одна пропорция на все
                     ширины — единственный способ выполнить просьбу
                     «показывать целиком».

                     Та же 3:2 стоит у карточки каталога
                     (`components/car-card.blade.php`) и у миниатюр
                     ниже. --}}
                <div class="relative mb-4 aspect-3/2 overflow-hidden rounded-card bg-photo">
                ...
                {{-- Миниатюры ОСТАЮТСЯ `object-cover`, хотя главный кадр
                     ...
                     А вот пропорция у них общая с главным кадром — 3:2
                     (правка 21.08.2026). Обрезка при этом не отменяется,
                     она просто перестаёт срабатывать на типовом снимке:
                     кадр в пропорцию плитки заполняет её ровно, резать
                     нечего. Файл другого формата подрезается как
                     и раньше. --}}
                @if ($car->photos->count() > 1)
                    <div class="grid grid-cols-5 gap-3">
                        @foreach ($car->photos as $index => $photo)
                            <a
                                ...
                                class="aspect-3/2 overflow-hidden rounded-field bg-photo transition"
