...
@section('content')
    ...
    {{--
        ...
        а не заводят разделы: якоря настоящие — до вехи 4.13 их выдавал
        `ServiceCategory::anchor()`, теперь это слаг строки справочника,
        и его существование проверяет `HomeContent`. Плашка ведёт к блоку —
        противоречия нет.
        ...
        <div class="mx-auto grid max-w-page items-start gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:gap-14">
            ...
            <div>
                ...
                {{-- Плашки — ссылки, а не карточки-иллюстрации: каждое
                     ...
                     ИКОНКИ ЗАПИСАНЫ СТРОКАМИ, и до вехи 4.13 это было
                     запрещено: три из них брались у `ServiceCategory`,
                     потому что плашка ведёт в блок категории, и значок
                     обязан был совпасть с тем, что человек увидит на той
                     стороне. Обоснование отменено вместе с иконками блоков
                     — заказчик просил снять их у заголовков категорий
                     19.08.2026, и совпадать теперь не с чем. Плашки значки
                     СОХРАНЯЮТ: это плашки направлений, а не заголовки
                     категорий, и «Поддержка» рядом с ними всегда стояла
                     с именем строкой.

                     АДРЕСА ДВУХ ПЛАШЕК РАЗРЕШАЕТ `HomeContent`, а не шаблон:
                     слаг категории правится из админки, то есть промах мимо
                     существующего блока стал достижимым состоянием, а ссылка
                     на несуществующий якорь не работает МОЛЧА. Промах даёт
                     ссылку на `/services` без якоря и WARN в лог. «Запчасти»
                     ведут на собственную страницу, а не на якорь, поэтому
                     разрешать там нечего; «Поддержка» категорией не является
                     вовсе.
                ...
                <div class="mt-9 grid gap-5 sm:grid-cols-2">
                    @foreach ([
                        ['icon' => 'wrench', 'title' => 'Автосервис', 'url' => $ecosystemLinks['maintenance'], 'text' => 'Диагностика, регламентные работы и подготовка автомобиля к эксплуатации.'],
                        ['icon' => 'sparkle', 'title' => 'Детейлинг', 'url' => $ecosystemLinks['detailing'], 'text' => 'Защитные покрытия, полировка, уход за салоном и кузовом.'],
                        ['icon' => 'gear', 'title' => 'Запчасти', 'url' => route('parts.index'), 'text' => 'Оригинальные детали и проверенные аналоги по VIN с понятным каналом заказа.'],
                    ...
                    <div class="grid gap-6 min-[900px]:grid-cols-2">
                        @foreach ($serviceGroups as $group)
                            <article class="rounded-card border border-line bg-surface p-7 lg:p-8">
                                {{-- Иконка в кружке снята вместе с вехой 4.13
                                     по просьбе заказчика: заголовки категорий
                                     на `/services` её лишились, и оставить её
                                     здесь значило бы развести два вида одного
                                     заголовка. Метод `ServiceCategory::icon()`
                                     удалён вместе с самим енамом. --}}
                                <div class="mb-2">
                                    <h3 class="text-[17px] font-semibold">{{ $group['category']->name }}</h3>
                                </div>
