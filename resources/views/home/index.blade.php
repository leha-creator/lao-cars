    ...
    @if ($steps !== [])
        ...
        <section id="process" class="theme-light pb-20 lg:pb-30">
            ...
            {{--
                ...
                а не «больше нуля»: прижатие ради 40 пикселей движения —
                это заикание, а не эффект.
            ...
            <div
                x-data="{
                    ...
                    /**
                     * Дистанция и решение о прижатии. Пересчитывается
                     * при изменении ширины окна: `travel` зависит от неё
                     * и через ширину карточки, и через ширину экрана.
                     */
                    measure() {
                    ...
                    follow() {
                        ...
                        this.$refs.track.scrollLeft = this.progress * this.travel;
                    },
            ...
            >
                ...
                >
                    ...
                    <div
                        ...
                        class="flex snap-x snap-mandatory gap-5 overflow-x-auto scroll-px-5 px-5 py-1 lg:snap-none lg:scroll-px-8 lg:px-8"
                    >
                        {{-- Карточки стояли на `page`, а не на `surface`, пока секция
                             была на `deep`: третий оттенок на экране был лишним.
                             Основание ушло вместе с `deep` 13.08.2026 — в светлой ветке
                             `page` у секции и `page` у карточки один и тот же цвет,
                             и карточка перестаёт читаться приподнятой: теней
                             в дизайн-системе нет, отделять её от фона больше нечем.
                             Отсюда `surface`. Иллюстрация внутри карточки третий
                             оттенок по-прежнему не заводит: кадр держит `bg-photo`,
                             тот же токен, что у каталога.

                             Ширина карточки — это и есть длина прокрутки, и обе цифры
                             подобраны под требование «не хочу, чтобы скролл отнимал
                             много времени». На 1440px шесть карточек по 22vw дают
                             ленту в 2065px, то есть 640px пути — меньше экрана;
                             на 1920px — около 400px. Цифры замерены, не оценены.
                             На 2560px лента (2324px) помещается в экран целиком,
                             и прижатия там не бывает по устройству, а не по ошибке:
                             карточка, гарантирующая пин на такой ширине, обязана быть
                             под 430px — то есть большой, чего просили не делать.

                             На мобильных 78vw — чтобы следующая карточка выглядывала
                             краем: без этого намёка ленту принимают за одну карточку
                             и не свайпают вовсе. --}}
                        @foreach ($steps as $step)
                            <article class="w-[78vw] max-w-90 shrink-0 snap-start overflow-hidden rounded-card border border-line bg-surface transition duration-250 hover:-translate-y-1 hover:border-accent/30 lg:w-[clamp(280px,22vw,360px)]">
                                @if ($step['image_url'] !== null)
                                    {{-- Пропорция задана контейнером и одинакова
                                         у всех шести: изображения разной высоты
                                         иначе разъехались бы по ленте. Приём тот же,
                                         что в `car-card`, — и по той же причине
                                         не нужны width/height, которых у записи
                                         библиотеки нет в известном виде.

                                         `3:2`, а не прежние `4:3`: карточка сузилась
                                         с ~460 до ~317px на 1440-м, и при прежней
                                         пропорции лента вытянулась бы по вертикали
                                         ровно там, где её просили сделать компактной.
                                    ...
                                    <div class="relative aspect-3/2 bg-photo">
                                        <img
                                            src="{{ $step['image_url'] }}"
                                        ...
                                        {{-- Номер переезжает на кадр, а не исчезает:
                                             ...
                                             НЕ переключаемая темой. До 13.08.2026
                                             здесь стояли `bg-deep/85` и `text-accent`:
                                             оба токена в светлой ветке меняются,
                                             и кружок стал бы почти белым пятном
                                             с тёмно-жёлтой цифрой — над светлым
                                             кадром он исчезает вовсе. Акцент поверх
                                             фотографии дизайн-система запрещает
                                             отдельным абзацем: 1.4:1 на светлом. --}}
                                        @if ($step['number'] !== null)
                                            <div class="absolute top-4 left-4 flex size-10 items-center justify-center rounded-full bg-scrim/85 font-display text-[15px] font-bold text-on-photo">
                                                {{ $step['number'] }}
                                            </div>
                                        @endif
                                    </div>
                                ...
                                <div class="p-7">
                                    ...
                                    @if ($step['text'] !== null)
                                        <p class="line-clamp-3 text-sm leading-[1.65] text-ink-muted">{{ $step['text'] }}</p>
                                    @endif
                                </div>
                            </article>
                        @endforeach
