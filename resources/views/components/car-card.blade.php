...
>
    {{-- Соотношение сторон держит контейнер, поэтому карточки не прыгают,
         ...
         `object-contain`, а не `object-cover` (веха 4.14): кадр
         вписывается целиком, поля закрывает `bg-photo`. Заказчик выбрал
         этот вариант из трёх 19.08.2026 — обрезка съедала у вертикальных
         снимков верх и низ.

         Контейнер `aspect-16/9` при этом ОСТАЁТСЯ, и это половина
         решения: карточка по пропорциям файла означала бы, что одна
         вертикальная фотография в ряду из трёх делает ряд вдвое выше
         и разносит сетку каталога. --}}
