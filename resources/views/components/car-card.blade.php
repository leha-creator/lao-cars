...
>
    {{-- Соотношение сторон держит контейнер, поэтому карточки не прыгают,
         ...
         Контейнер при этом ОСТАЁТСЯ, и это половина решения: карточка
         по пропорциям файла означала бы, что одна вертикальная
         фотография в ряду из трёх делает ряд вдвое выше и разносит
         сетку каталога.

         Пропорция контейнера — 3:2 (правка 21.08.2026), и она базовая:
         в этом формате снимают автомобили, а у кадра в пропорцию
         контейнера поля `bg-photo` схлопываются в ноль — снимок
         заполняет карточку целиком, без полей и без обрезки. Прежние
         16:9 давали тонкие полосы сверху и снизу КАЖДОМУ снимку
         типовой съёмки, то есть поля были правилом, а не исключением.
         Теперь они остаются ровно там, где и задуманы вехой 4.14, —
         у кадра другого формата.

         Та же 3:2 стоит у главного кадра и у миниатюр на странице
         автомобиля. Базовая пропорция обязана быть одна на весь сайт:
         иначе один и тот же файл получает поля в списке и не получает
         на странице, и «правильный формат» перестаёт существовать
         как понятие. --}}
    <div class="aspect-3/2 bg-photo">
