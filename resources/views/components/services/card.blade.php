...
{{--
    ...
    ниже `p-7` с текстом): прецедент работает на светлой ветке с вехи 4.12,
    и брать готовое дешевле, чем изобретать третий вид карточки на одной
    странице.

    КАДР ВПИСЫВАЕТСЯ, А НЕ ОБРЕЗАЕТСЯ (веха 4.14). Изначально веха трогала
    только карточки автомобилей — заказчик говорил про них, — и здесь
    осознанно оставался `object-cover`. Решение пересмотрено на приёмке:
    посетитель видел вписанный кадр в каталоге и обрезанный в услугах,
    и эту несогласованность он замечает раньше нас, а болезнь у неё та же
    — у вертикального снимка исчезают верх и низ.

    Пропорция переехала С САМОГО `img` НА КОНТЕЙНЕР, и это не косметика:
    `object-contain` оставляет поля, а закрасить их может только элемент
    под кадром. На `img` без контейнера поля взяли бы цвет карточки,
    и снимок выглядел бы не вписанным, а случайно уменьшенным. Контейнер
    при этом держит сетку ровной независимо от пропорций файла — то же
    основание, что у `x-car-card`.
...
>
    @if ($service->imageUrl !== null)
        ...
        <div class="aspect-4/3 w-full bg-photo">
            <img
    ...
    <div class="flex flex-1 flex-col p-7">
        ...
        @if ($service->hasPrice() || $priceFallback)
            {{-- «По запросу» — не цена, и набирать её акцентом наравне
                 ...
                 Кегль вдвое крупнее прежнего — по просьбе заказчика, на
                 обеих страницах, где живёт компонент («Сервис»
                 и «Запчасти»). Переносу место есть: `div` без
                 `whitespace-nowrap`, и диапазон шире колонки уходит на
                 вторую строку, а не за край карточки. --}}
            <div @class([
                'mt-4 text-[28px] leading-[1.15] text-balance',
