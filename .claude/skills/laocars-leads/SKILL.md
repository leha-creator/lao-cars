...
автосервиса, на посадочной запчастей, в контактах — создаёт одну и ту же сущность `Lead`.
Отличается только источник. Заявка на подбор запчасти — тоже `Lead`: источником выступает
`Service` из категории `parts`, отдельной сущности под запчасти в MVP нет.
...
Это минимальный набор. Веха 3.7 роадмапа добавляет к нему поля из макета (способ связи,
удобное время звонка) и поля подбора запчасти (марка, модель, VIN) — все nullable, потому
что заполняются не каждой формой. Актуальный состав полей — в `.ai-factory/ROADMAP.md`.
