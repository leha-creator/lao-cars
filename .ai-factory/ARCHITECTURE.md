...
Выбор продиктован реальностью проекта, а не модой: сайт ЛАО КАРС — это в основном CRUD (каталог, услуги, блог, команда, отзывы) плюс один по-настоящему логичный путь — приём заявки. Штатный скелет Laravel уже является слоистым, а Filament жёстко ожидает свои ресурсы в `app/Filament/Resources`. Городить поверх этого модули или Domain/Application/Infrastructure значило бы воевать с фреймворком и админкой ради структуры, которую нечем наполнить.
...
laocars/
├── app/
│   ├── Enums/                          # LeadStatus, CarStatus, ServiceCategory
│   ├── Filament/
│   │   ├── Resources/                  # Админка: CarResource, ServiceResource,
│   │   │   └── ...                     # LeadResource, ArticleResource, ...
...
- ❌ Модель не знает о сервисах, контроллерах и HTTP-запросе
...
Layered выбран под MVP. Сигналы, что пора переходить к Structured Modules (модули Catalog / Services / Blog / Leads):
