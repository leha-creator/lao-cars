...
│   │                                   # (hero-960/1920.webp) и фон секции
│   │                                   # заявки (lead-bg.webp). Попадает
...
Граница проходит именно там, где раньше шло ветвление: **вокруг** `<form>`. Внутренности формы инвариантны с вехи 3.7, и сторож здесь тот же — `LeadStoreTest` и `SectionPagesTest`, прошедшие без правок. Одной формы на три сценария это не отменяет: копии `<form>` по-прежнему нет, а поле, добавленное в одну копию из двух, теряло бы данные молча.
