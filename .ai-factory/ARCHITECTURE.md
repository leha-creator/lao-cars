...
**Про параметр против второго компонента.** У `x-lead-form` две раскладки (веха 4.2): центрированная карточка на страницах разделов и две колонки на фоновом фото на главной. Это параметры (`eyebrow`, `heading`, `text`, `background`), а не второй компонент, по тому же основанию, по которому веха 3.7 сделала одну форму на три сценария: копия разъедется на первом изменении набора полей, а поле, добавленное в одну копию из двух, теряет данные молча. Ветвление идёт **вокруг** `<form>` — внутренности формы инвариантны, и сторож здесь `LeadStoreTest` с `SectionPagesTest`, прошедшие без правок. Триггер пересмотра назван: третий вызов с раскладкой — повод вынести обёртку в отдельный компонент, оставив `x-lead-form` только формой.
