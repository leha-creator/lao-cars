...
it('fits the card photo instead of cropping it', function () {
    ...
    // с фиксированной пропорцией при этом ОСТАЁТСЯ — карточка по
    // пропорциям файла означала бы, что одна вертикальная фотография
    // в ряду из трёх делает ряд вдвое выше и разносит сетку.
    //
    // Пропорция контейнера — 3:2, и это не оформление, а вторая половина
    // той же просьбы (заказчик, 21.08.2026): «большинство картинок будут
    // такого формата, они должны показываться полностью, без полей;
    // поля — если загружено фото другого формата». С `object-contain`
    // ширину полей задаёт именно пропорция контейнера: у кадра 3:2 она
    // нулевая, у любого другого — нет. Прежние 16:9 давали полосы
    // КАЖДОМУ снимку типовой съёмки. Та же 3:2 стоит на странице
    // автомобиля — сторож там, в `CatalogShowTest`.
    $car = Car::factory()->create();
    CarPhoto::factory()->for($car)->create();

    $content = $this->get('/catalog')->assertOk()->getContent();

    preg_match('/<div class="aspect-3\/2 bg-photo">\s*<img[^>]*>/s', $content, $card);
