...
├── config/home.php           # Лимиты блоков главной: отзывов и позиций
│                             # прайса на категорию (веха 4.6)
...
│   │                         # @layer base — тёмная тема по умолчанию плюс
│   │                         # .theme-light: светлые секции вехи 4.11
│   │                         # переопределением токенов на поддереве
...
│       │                     # вехой 4.6 по макету v2 (14 блоков)
│       ├── errors/           # 404 и 500 (веха 4.11). 404 на общем layout;
...
- **Палитра Tailwind по умолчанию обнулена** (`--color-*: initial`, веха 4.3): классов вида `text-gray-400` в сборке нет, цвет берётся только из токенов. `--color-white` и `--color-black` возвращены вручную сразу за обнулением — они нужны вью панели, которые доезжают в публичный бандл через `@source`, и без них разделители там пропадают молча.
- **Публичные шаблоны не пишут литерал `white` и не заливают `bg-accent`** (веха 4.11). Границы — токены `border-line` / `-strong` / `-loud`, подложка — `bg-overlay`, акцентная ЗАЛИВКА — `bg-accent-solid`, текст на ней — `text-on-accent`. Причина в светлой теме: `.theme-light` переопределяет токены на своём поддереве, а литерал и неизменную заливку переопределить нечем — в светлой секции получится невидимая граница или тёмная надпись на тёмном. Сборка при этом зелёная. Сторож — `tests/Feature/View/PaletteGuardTest.php`.
