...
| Заявки и уведомления | docs/leads.md | Путь заявки, форматы ответа, согласие на обработку ПДн, выключатель приёма, защита, бот, работа менеджера |
| Прод и деплой | docs/deploy.md | Устройство прода, выкат и откат, секреты, разовые команды, бэкапы, мониторинг |
...
- **Состав меню правится в одном месте** — `app/Support/SiteMenu.php`. Он же расходится с макетом намеренно: там «Услуги» и «Блог», в проекте «Сервис» и «Запчасти». Подписи пунктов расходятся и с названиями разделов: заказчик переименовал «Каталог» → «Автомобили» и «Автосервис» → «Сервис» в меню, а H1 страниц, хлебные крошки с их JSON-LD и вкладку админки не трогал — это не забытые места, разбор в docblock `SiteMenu`.
